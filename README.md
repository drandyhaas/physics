...
| [**EMviewer3D**](EMviewer3D/) | **Field bench 3D** — the same circuit with the plane restriction lifted, so both the loop and the field live in space. Orbit, zoom, and push a slice plane through the field; field lines are traced in three dimensions rather than on the plane. Shows **B** as the closed rings threaded on the wire that a planar picture can only represent as ⊗ and ⊙ marks, and gathers them into a bundle for a solenoid. Its charge solve is checked against the planar one, and its Biot–Savart against brute-force integration. |
| [**EMviewer4D**](EMviewer4D/) | **Switch-on bench** — the same circuit, with time. The battery comes on over a few nanoseconds and the field has to catch up: a front spreading at *c* with nothing at all outside it, a current held back from the EMF by the loop's own inductance, and energy arriving only where both fields already are. The inductance is computed from the geometry rather than assumed, and agrees with the thin-ring formula to 2%; the fields come from Jefimenko's equations at retarded time, so causality is never imposed. |
